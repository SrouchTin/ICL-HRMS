                ...
                <div class="px-8 py-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm text-slate-500 mt-1">Manage your personal time off requests</p>
                        </div>
                            ...
                            <table class="w-full min-w-max">
                                <thead>
                                    <tr class="bg-slate-50 border-b border-slate-200">
                                        <th class="px-6 py-4 text-center text-xs font-semibold text-slate-600 uppercase tracking-wider w-16">
                                            No.
                                        </th>
                                ...
                                <tbody class="divide-y divide-slate-200">
                                    @forelse($leaves as $leave)
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                @if($leave->leave_for === 'half_day')
                                                    <span class="inline-flex px-3 py-1 rounded-full text-xs font-semibold
                                                        {{ $leave->half_day_type === 'morning' ? 'bg-blue-100 text-blue-800' :
                                                           ($leave->half_day_type === 'afternoon' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800') }}">
                                            ...
                                            <!-- Reject Reason -->
                                            <td class="px-6 py-4 text-red-700 font-medium no-wrap" title="{{ $leave->reject_reason }}">
                                                {{ $leave->reject_reason ? Str::limit($leave->reject_reason, 40) : '—' }}
                                            </td>

                                            <!-- Status -->
                                            <td class="px-6 py-4 text-center">
                                                @if($leave->status === 'Approved')
                                                    <span class="inline-flex px-4 py-2 rounded-full text-sm font-bold bg-green-500 text-white">APPROVED</span>
                                                @elseif($leave->status === 'Pending')
                                                    <span class="inline-flex px-4 py-2 rounded-full text-sm font-bold bg-amber-500 text-white">PENDING</span>
                                                @elseif($leave->status === 'Rejected')
                                                    <span class="inline-flex px-4 py-2 rounded-full text-sm font-bold bg-red-500 text-white">REJECTED</span>
                                                @else
                                                    <span class="text-slate-400">—</span>
                                                @endif
                                            </td>
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                <div class="flex items-center justify-center gap-4">
                                                    @if($leave->status === 'Pending')
                                                        <a href="{{ route('hr.own-leave.edit', $leave->id) }}"
                                                            class="text-blue-600 hover:text-blue-800">
                                                            <i class="fas fa-edit text-lg"></i>
                                                        </a>
                                                        <form action="{{ route('hr.own-leave.destroy', $leave->id) }}"
                                                    ...
                                                    @else
                                                        <i class="fas fa-edit text-lg disabled-icon" title="Cannot edit after processing"></i>
                                                        <i class="fas fa-trash text-lg disabled-icon" title="Cannot delete after processing"></i>
                                                    @endif
                                                </div>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="14" class="px-6 py-20 text-center text-slate-500">
